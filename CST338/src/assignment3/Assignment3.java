...
//         System.out.println("Invalid Input");
	...
	public boolean isValid(char value, Suit newSuit) {
		...
		if (compareChar.equals('A') || compareChar.equals('2') || compareChar.equals('3') || compareChar.equals('4')
			|| compareChar.equals('5') || compareChar.equals('6') || compareChar.equals('7') || compareChar.equals('8')
			|| compareChar.equals('9') || compareChar.equals('T') || compareChar.equals('J') || compareChar.equals('Q')
	...
	boolean equals(Card card) {
		...
		if (charToStringThis.equals(charToStringInput) && suit.equals(card.suit) && errorFlag == card.errorFlag) {
			// included for testing purposes
			System.out.println("The values compared with equals() are equal.");
			System.out.println("The calling object values are: " + suit.toString() + " " + charToStringThis);

			System.out
				.println("The object passed to method values are: " + card.suit.toString() + " " + charToStringInput);
			//
		...
		System.out.println("The object passed to method values are:" + card.suit.toString() + " " + charToStringInput);
...
class Hand {
	...
	/**
	 * Returns the top card in the array, then decrements the number of cards in the
	 * array to "remove" that top card.
	 * 
...
class Deck { // The Deck class will create two decks of cards.
...
// the new position of our card swap location.
		...
		while (topCard != 0) { // while the top card is not the last card in deck,
			int randCard = randPick.nextInt(topCard + 1); // Pick a random number
			Card tempCard = cards[topCard]; // Use a temporary card object to assist with the swap
			cards[topCard] = cards[randCard]; // After holding the topCard object in a temporary card object,
			...
			cards[randCard] = tempCard; // The object that used to be in the random index location is gone, so this
			// assign the card object we saved in tempCard to the location of the random
			// index.
			// By using the temporary object, we did not destructively copy over any objects
...
// Thus, we were out of cards to shuffle at that point.
	...
	public int getTopCard() { // An accessor for the int, topCard (no mutator)
	...
	private static void allocateMasterPack() { // This creates new master deck
		if (masterPack != null) { // "Have I been here before?"
			return; // return if masterPack already exists
		}
		int numCards = 52 * numPacks;
		if (numCards > MAX_CARDS) { // return if masterPack has too many cards
